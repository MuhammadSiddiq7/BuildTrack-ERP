...
@endif
    ...
    {{-- <p>B. Addendum A to MOU No. {{ $poItem->supplier->mou_no ?? '-' }} {{ $poItem->supplier->addendum_no ?? 'N/A' }} date:
    {{ $poItem->supplier->addendum_date ? \Carbon\Carbon::parse($poItem->supplier->addendum_date)->format('d F Y') : 'N/A' }}  
   </p>     --}}
...
<div class="content">
  ...
  <table class="items" aria-label="items table">
   ...
   <tbody>
      ...
      <tr>
        <td class="center">{{ $loop->index == 0 ? 'a.' : $loop->iteration }}</td>
        <td>{{ $item->item->item ?? '-' }}</td>
        <td class="center">{{ $item->item->deno ?? '-' }}</td>
        <td>
          @if($poItem->demand->contractors->isNotEmpty())
              {{ $poItem->demand->contractors->first()->name }}
          @else
              -
          @endif
        </td>
